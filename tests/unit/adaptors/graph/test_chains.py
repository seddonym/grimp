...
class TestFindShortestChains:
    ...
    def test_top_level_import(self, as_packages: bool, expected_result: Set[Tuple]):
        graph = ImportGraph()
    ...
    def test_no_results_in_reverse_direction(self, as_packages: bool, expected_result: Set[Tuple]):
        graph = ImportGraph()
        graph.add_module("green")
        graph.add_module("blue")
        graph.add_import(importer="green.foo", imported="blue.bar")
