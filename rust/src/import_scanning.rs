...
impl ImportScanner {
    ...
    fn scan_for_imports<'a>(
        ...
        match parse_result {
            Err(GrimpError::ParseError { line_number, text, ..}) => {
                // TODO: define SourceSyntaxError using pyo3.
                let exceptions_pymodule =
                    PyModule::import(py, "grimp.exceptions").unwrap();
                let py_exception_class = exceptions_pymodule.getattr("SourceSyntaxError").unwrap();
                let exception = py_exception_class.call1((module_filename, line_number, text,)).unwrap();
...
impl ImportScanner {
    ...
    fn _determine_module_filename(
        ...
        let normal_module_path = format!("{}.py", filename_root);
    ...
    fn _to_py_direct_imports<'a>(
        ...
        let valueobjects_pymodule =
            PyModule::import(py, "grimp.domain.valueobjects").unwrap();
...
fn count_leading_dots(s: &str) -> usize {
    s.chars()
        .take_while(|&c| c == '.') // Take characters as long as they are dots
        .count() // Count how many were taken
}
